...
export function InputIcon({ error = false, ...props }: InputIconProps) {
  return (
    <span
      className="text-gray-400 group-focus-within:text-gray-100 group-[&:not(:has(input::placeholder-show))]:text-gray-100 group-data-[error=true]:text-danger"
      {...props}
    />
  )
}
...
export function InputField({ error = false, ...props }: InputFieldProps) {
  return (
    <div
      data-error={error}
      className="group bg-gray-800 w-full rounded-xl px-4 flex items-center gap-2 focus-within:border-gray-100 data-[error=true]:border-danger"
    >
